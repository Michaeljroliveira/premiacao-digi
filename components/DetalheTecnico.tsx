...
export default function DetalheTecnico({
  ...
  // Cálculo do Potencial Adicional
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="rounded-xl bg-green-50 p-4">
              <p className="text-gray-500 text-sm">Prêmio Total</p>
          ...
          {/* NOVO: Card do Potencial Adicional */}
          <div className="mt-4 rounded-xl bg-amber-50 border-2 border-amber-400 p-5">
            ...
            <p className="text-xs text-gray-600 mt-2">
              Diferença entre o que ganhaste e a meta de 4 instalações/dia.
            </p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            <div className="rounded-lg bg-yellow-100 p-3 text-center">
              <p className="text-xs text-yellow-800">5+ instalações</p>
              <p className="text-2xl font-bold text-yellow-700">{diasCom5mais}</p>
            </div>
            <div className="rounded-lg bg-green-100 p-3 text-center">
              <p className="text-xs text-green-800">4 instalações</p>
              <p className="text-2xl font-bold text-green-700">{diasCom4}</p>
            </div>
            <div className="rounded-lg bg-blue-100 p-3 text-center">
              <p className="text-xs text-blue-800">3 instalações</p>
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {historico.map((dia) => {
                const faltamParaMeta = Math.max(0, META_DIARIA - dia.instalacoes);
                const potencialDia =
                  ...
                  >
                    ...
                    {potencialDia > 0 && (
                      <div className="mt-3 text-sm text-amber-700 bg-amber-50 rounded-lg p-2">
                        💡 Potencial: <strong>€ {potencialDia.toFixed(2)}</strong>
                        {faltamParaMeta > 0 && (
                          <>
                            {" "}— faltaram{" "}
                            <strong>{faltamParaMeta}</strong>{" "}
                            instalação(ões) para atingir a meta de 4.
                          </>
                        )}
                      </div>
                    )}
